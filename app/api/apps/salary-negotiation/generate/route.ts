...
// Mirrors the lightweight in-memory pattern used elsewhere (e.g. TropicAtlas
// /api/track). NOTE: this is per-server-instance and resets on cold start —
// fine as a cheap abuse guard, not a hard global quota. Swap for a shared
// store (Upstash/Redis) if strict global enforcement is ever needed.
const RATE_LIMIT = 5;
...
// consume. (No min/max/length constraints — those aren't supported by the API
...
export async function POST(request: Request) {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    console.error("ANTHROPIC_API_KEY is not set — salary-negotiation generation cannot run.");
    return NextResponse.json(
  ...
  // 7. Call Claude with structured output.
